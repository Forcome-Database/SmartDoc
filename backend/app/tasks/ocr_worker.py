...
class OCRWorker:
        ...
        async with SessionLocal() as db:
            try:
                ...
                try:
                    ...
                    # 记录数据提取阶段的LLM消耗
                    extraction_llm_stats = extraction_result.get(
                        'llm_stats', {})
                    total_llm_tokens = extraction_llm_stats.get(
                        'token_count', 0)
                    ...
                    if rule_config.get('llm_config', {}).get('enabled', False):
                        ...
                        # 累加LLM增强阶段的Token消耗
                        total_llm_tokens += enhancement_llm_stats.get(
                            'token_count', 0)
                        total_llm_cost += enhancement_llm_stats.get(
                            'cost', 0.0)
                    ...
                    logger.info(
                        f"任务LLM消耗统计: Token={total_llm_tokens}, 费用=¥{total_llm_cost:.4f}")
                    ...
                    if needs_audit:
                        ...
                        await self._send_dingtalk_notification(
                            task, rule_config, validation_result
                        )
                    else:
                        # 更新状态为已完成
                        await self._update_task_status(
    ...
    async def _load_rule_config(
        ...
        try:
            ...
            # 确定要使用的版本
            target_version = rule_version or rule_obj.current_version
            ...
            # 合并basic配置，确保包含规则名称
            basic_config = version_config.get('basic', {})
            basic_config['name'] = rule_obj.name  # 从规则对象获取名称
            ...
            logger.info(
                f"规则配置加载成功: {rule_id} - {rule_obj.name} (版本: {target_version})")
            logger.info(
                f"提取规则数量: {len(extraction_rules)}, 清洗规则数量: {len(cleaning_rules)}, 校验规则数量: {len(validation_rules)}")
    ...
    async def _execute_ocr(
        ...
        try:
            ...
            if page_strategy_mode == 'specified_pages':
                page_strategy['page_range'] = basic_config.get(
                    'pageRange', '1')
            ...
            logger.info(
                f"开始OCR处理: 引擎={engine}, 语言={language}, 页面策略={page_strategy}, fallback=disabled")
    ...
    async def _execute_extraction(
        ...
        try:
            ...
            logger.info(
                f"数据提取完成: {len(extracted_data)} 个字段, LLM Token消耗: {llm_stats.get('token_count', 0)}")
    ...
    async def _execute_llm_enhancement(
        ...
        try:
            ...
            confidence_scores = extracted_data.get(
                'confidence_scores', {}).copy()
            ...
            for field_config in llm_fields:
                ...
                if llm_result:
                    ...
                    confidence_scores[field_name] = llm_result.get(
                        'confidence', 0.0)
    ...
    async def _execute_validation(
        ...
        try:
            ...
            auto_required_rules = self._generate_required_rules_from_schema(
                schema)
            ...
            logger.info(
                f"开始数据校验: {len(all_validation_rules)} 个规则 (自动必填: {len(auto_required_rules)}, 用户配置: {len(validation_rules)})")
            ...
            if node_type == 'object' and 'properties' in field_def:
                ...
                if items:
                    ...
                    if required_child_fields:
                        ...
                        logger.debug(
                            f"自动生成数组子字段必填校验规则: {field_path}, 必填字段: {required_child_fields}")
    ...
    async def _save_results(
        ...
        try:
            ...
            ocr_result_to_save = {
                k: v for k, v in ocr_result.items() if k != 'merged_text'}
            ...
            if rule_config:
                schema = rule_config.get('schema', {})
                # 全局默认阈值（当字段未配置时使用）
                default_threshold = rule_config.get(
                    'audit_config', {}).get('confidence_threshold', 80.0)
    ...
    def _get_field_confidence_threshold(
        ...
        if not field_name or not schema:
            logger.debug(
                f"字段 {field_name} 使用默认阈值: {default_threshold} (schema为空)")
        ...
        if field_name in schema:
            ...
            logger.debug(
                f"字段 {field_name} Schema配置: {field_def}, 阈值: {threshold}")
            return threshold
    ...
    async def _send_dingtalk_notification(
        ...
        try:
            ...
            # 置信度过低
            if hasattr(task, 'audit_reasons') and task.audit_reasons:
                for reason in task.audit_reasons:
                    if reason.get('type') == 'confidence_low':
                        audit_reasons.append(reason)

            # 获取规则名称
            rule_name = rule_config.get('basic', {}).get('name', '未知规则')

            # 发送钉钉通知（传入rule_id用于规则过滤）
            await dingtalk_service.notify_pending_audit(
                task_id=str(task.id),
